...
public class NoFila implements Comparable<NoFila> {
    ...
    private int     ttl;
    private int[]   vetorCaminhoTarefa;  //caminho que a tarefa tera que,
    // eventualmente, percorrer devido ao escalonamento/roteamento
    ...
    private boolean finalizada; //true = esta finalizada; false = nao esta
    // finalizada

    public NoFila (
            int idTarefa, int idCS, int idFila, int idServ, Double tamProc, Double tempoCFProc, Double tamCom,
            Double tempoCFCom
    ) {
        setIdTarefa(idTarefa);
        setIdCSAtual(idCS);
        setIdCSDestino(-1); //Na criacao da tarefa, o CS atual tambem e o CS
        // de destino
        setIdFila(idFila);
        setIdServidorAtual(idServ);
        setIdServidorDestino(-1);  //Na criacao da tarefa, o servidor atual
        // tambem e o servidor de destino
    ...
    public int getElementoVetorCaminhoTarefa (int i) {
        ...
        if (i < vetorCaminhoTarefa.length) {return vetorCaminhoTarefa[i];}
    ...
    public void imprimeDadosTarefa () {
        ...
        if (vetorCaminhoTarefa != null) {
            for (i = 0; i < vetorCaminhoTarefa.length; i++) {System.out.printf("%d ", vetorCaminhoTarefa[i]);}

        }
        System.out.printf("\n");
        if (getFinalizada() == true) {System.out.printf("finalizada = true\n");} else {
