...
public class CentrosDeServico {
    ...
    public CentrosDeServico (
            int identCS, int tp, int nMaxServ, int nMaxFilas,
    ...
    public CentrosDeServico (int identCS, int tp, int nMaxServ, int nMaxFilas) {
        ...
        setNumMaxFilas(nMaxFilas);
        setNumAtualFilas(0);
        if (tp == 1) {setIndiceEscravoAtualRRCluster(1);}
        servidores = new HashSet<Servidores>();
        filas      = new HashSet<Filas>();
    }
    ...
    public void adicionaServidorProcto (
        ...
        if (msOuEsc == true) {ehEscalonador = true;} else {ehEscalonador = false;}

        if (getEhEscalonador() == false) {System.out.printf("o CS %d NAO eh escalonador\n", getIdCS());} else {
    ...
    public void adicionaServidorCom (
            int tpServ, Double tServRede, double ltnc
    ...
    public void adicionaServidoresInt (
            int tpServ, Double tServRede,
            ...
            for (Servidores servTemp : servidores) {
                if (servTemp.getIdServidor() == 0) {
                    ...
                    atualCS.addNoMedidasServidor(0, 1,
                                                 tarefa.getTamanhoComTarefa() / servTemp.getTServicoRede() +
    ...
    public Double atribuiTarefaServProc (
            NoFila tarefa,
        ...
        if (getTipo() == 0) {
            for (Servidores servTemp : servidores) {
                if (servTemp.getIdServidor() == 0) {
                    retornoFuncao = servTemp.atribuiTarefaServProc(
                            tarefa,
                    ...
                    servTemp.setTempoPrevistoLiberacao(
                            instanteOcupacaoServidor + tarefa.getTamanhoProcTarefa() / servTemp.getTServicoProcto());
    ...
    */
    ...
    public boolean verificaFilaComunVazia (int tipoCS) {
        ...
        if (tipoCS == 1) {
            for (Filas temp : filas) {
                if (temp.getIdFila() == 1) {
                    tamFila = temp.size();
                    if (tamFila == 0) {retornoFuncao = true;} else {retornoFuncao = false;}
                }
            }
        } else {
            for (Filas temp : filas) {
    ...
    public Double atribuiTarefaMestre (
            NoFila tarefa, int idCS,
            MetricaCS atualCS
    ) {
        Double retornoFuncao = 0.0;
    ...
    public int verificaSeHaEscravoLivre () {
        ...
        System.out.printf(
                "indiceEscravoAtualRRCluster(fora) = %d\n",
    ...
    */
    ...
    public boolean verificaSeServidorEstaLivre (NoFila tarefa) {
        boolean retornoFuncao = false;
        for (Servidores servTemp : servidores) {
            if (servTemp.getIdServidor() ==
                tarefa.getIdServidorAtual()) {   //System.out.printf("********servTemp.getIdServidor() = %d\n",
                // servTemp.getIdServidor() );
                retornoFuncao = servTemp.getServidorLivre();
            }
        }
