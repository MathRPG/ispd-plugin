...
// rede de filas----------------------------+
	...
	*/
	...
	/*	public void adicionaServidoresInt( int idCS, int tpServ, Double
	tServRede, int tpDistCom, double ltnc, double txOcup )	{   for(
	CentrosDeServico csTemp : centroDeServicos )
	    {   if( csTemp.getIdCS() == idCS )
		    {   csTemp.adicionaServidoresInt( tpServ, tServProctoTotal, csTemp
		    .getNumMaxServidores(), tpDistrProcto );
			}
		}
	}
*/
        ...
        for (CentrosDeServico csTemp : centroDeServicos) {
            if (csTemp.getIdCS() == tarefa.getIdCSAtual()) {
                ...
                // cluster, faz escalonamento interno
                {
					...
					servidor mestre e sempre
					0, os ids dos escravos sao maiores ou iguais a 1. Logo, so
					 faco escalonamento se
    ...
    public Double saidaTarefaServComunHopFinal (NoFila tarefa) {
        ...
        if (tarefa.getTtl() <= 1) {
            ...
            /*
			   Esse if tem que ser feito, pois como ha dois tipos de mestre -
			   (um de cluster de
CS tipo 1 e outro de cluster de maquinas (CSs tipo 0 para processamento e CSs
 tipo 2 ou 3 para rede) ) -,
a saida da tarefa de um mestre pode, tambm, ocorrer de duas formas:
            - Se o mestre e de um CS tipo 1, a tarefa sai do mestre, mas
            continua no CS, ou seja
			nao se executa incrementaIndiceProximoHop() na funcao
			saidaTarefaMestre() (no trecho getTipoCS == 1)
			- Se o mestre e de um CS tipo 0, a tarefa sai do mestre E do CS,
			entao e necessario
			se executar incrementaIndiceProximoHop() na funcao
			saidaTarefaMestre() (no trecho getTipoCS == 0)
			Isso pode acarretar uma "defasagem" no indiceProximoHop que, se
			...
			*/
			...
			/*ATENCAO: perceber que o resultado do if abaixo sera diferente do
			 resultado do if anterior
			pois, devido a preparacao comentada no comentario anterior, eu
			mudei, na linha acima,
			o IdCSAtual da tarefa! Antes, dessa mudanca a variavel continha o
			Id do CS antigo,
			agora contem o id do proximo CS. Apos a "chamada de atencao
			anterior, e conveniente explicar
			a funcao/proposito do for/if abaixo. Para "setar" o servidor que a
			 tarefa sera alocada e necessario
			saber se o servidor de processamento eh uma maquina (CS tipo 0) ou
			 eh um no (CS tipo 1), pois
            se for uma maquina, o id deste servidor sera 0 (pois, apesar de
            ser um escravo, eh o unico
			na maquina), se for de um cluster, o id do servidor estara
			armazenado na variavel idServidorDestino
            da tarefa.
            */
            for (CentrosDeServico csTemp : centroDeServicos) {
                if (csTemp.getIdCS() == tarefa.getIdCSAtual()) {
        ...
        for (i = 0; i <= numCS; i++) {
            for (CentrosDeServico csTemp : centroDeServicos) {
                if (csTemp.getIdCS() == i) {
                    ...
                    switch (csTemp.getTipo()) {
                        case 0:
                            System.out.printf("Tipo: maquina\n");
                            break;
                        case 1:
                            System.out.printf("Tipo: cluster\n");
                            break;
                        case 2:
                            System.out.printf("Tipo: Conexao rede\n");
                            break;
                        case 3:
                            System.out.printf("Tipo: Internet\n");
                            break;
                    }
