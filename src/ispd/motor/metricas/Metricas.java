...
//Consumo máximo da porção de cada usuário, como se a porção ficasse ativa por completo durante toda a simulação
...
//Energia consumida na porção com tarefas de usuários não proprietários
    ...
    public Metricas (RedeDeFilas redeDeFilas, double time, List<Tarefa> tarefas) {
        ...
        for (CS_Processamento maq : redeDeFilas.getMaquinas()) {
            ...
            consumoMaxLocal.put(
                    propMaq, consumoMaxLocal.get(propMaq).add((BigDecimal.valueOf(maq.getConsumoEnergia()))));

            limitesConsumoTempoSim.put(
                    propMaq, limitesConsumoTempoSim.get(propMaq).add((
                                                                             BigDecimal.valueOf(maq.getConsumoEnergia())
                                                                     )));
        ...
        for (String user : usuarios) {
            ...
            limitesConsumoTempoSim.put(
                    user, limitesConsumoTempoSim.get(user).multiply(BigDecimal.valueOf(porcentLimite)));
    ...
    private void getMetricaFilaTarefa (List<Tarefa> tarefas, RedeDeFilas rede) {
        ...
        for (Tarefa tar : tarefas) {
            if (tar.getEstado() == Tarefa.CONCLUIDO) {
                ...
                BigDecimal tempoInicio    =
                        BigDecimal.valueOf(tar.getTimeCriacao());//Instante de tempo de submissão da tarefa
                BigDecimal tempoFinal     = BigDecimal.valueOf(tar.getTempoFinal().get(tar.getTempoFinal().size() -
                                                                                       1));//Instante de tempo em que
                // a terafa é concluída
                BigDecimal intervaloReal  = tempoFinal.subtract(tempoInicio);//Intervalo entre término e submissão
                BigDecimal intervaloIdeal = BigDecimal.valueOf(tar.getTamProcessamento() /
                                                               mediaPoder);//Tempo de execução esperado pelo usuário,
                // em que não há espera nem preempção, com a tarefa executando em uma máquina média do sistema
                ...
                if (satisfacaoGeralSim.putIfAbsent(propTar, suij) !=
                    null) {//Faz a primeira incersão testando se o hashmap está vazio e, se não estiver, entra no
                    // corpo no if
                ...
                if (satisfacaoDesempenho.putIfAbsent(propTar, suij) !=
                    null) {//Faz a primeira incersão testando se o hashmap está vazio e, se não estiver, entra no
                    // corpo no if
                ...
                if (tar.getHistoricoProcessamento().size() >
                    1) {//Se a tarefa passou por mais de uma máquina, houve preempção dela
                    if (tarefasPreemp.putIfAbsent(propTar, 1) !=
                        null) {//Faz a primeira incersão testando se o hashmap está vazio e, se não estiver, entra no
                        // corpo no if
                    ...
                    for (i = 0; i < tar.getHistoricoProcessamento().size(); i++) {
                        ...
                        energiaDespercicada.put(
                                propTar, energiaDespercicada.get(propTar).add(BigDecimal.valueOf(tempoDesperdicio * tar
        ...
        for (int i = 0; i < usuarios.size(); i++) {
            ...
            satis = satisfacaoGeralSim.get(user)
                                      .divide(BigDecimal.valueOf(tarefasConcluidas.get(user)), 2, RoundingMode.DOWN);
            ...
            limitesConsumoTempoSim.put(
                    user, limiteConsSim.multiply(BigDecimal.valueOf(metricasGlobais.getTempoSimulacao())));
            ...
            for (int j = 0; j < tarefas.size(); j++) {
                ...
                if (tarefas.get(j).getProprietario().equals(user)) {
                    ...
                    turnaroundTime.put(user, turnaroundTime.get(user).add(BigDecimal.valueOf(
            ...
            BigDecimal alpha;
            if (consumoLocal.get(user).compareTo(BigDecimal.ZERO) == 0) {alpha = BigDecimal.ZERO;} else {
                alpha = consumoMaxLocal.get(user).divide(consumoLocal.get(user), 2, RoundingMode.DOWN);
            }

            BigDecimal betaSim;
            if (limitesConsumoTempoSim.get(user).compareTo(BigDecimal.ZERO) == 0) {betaSim = BigDecimal.ZERO;} else {
                betaSim = (
                        ((consumoEnergiaTotalUsuario.get(user).negate()).add(consumoTotalSistema)).divide(
                                limitesConsumoTempoSim.get(user), 2, RoundingMode.DOWN)
                ).add(BigDecimal.ONE);
            }
            ...
            turnaroundTime.put(user, turnaroundTime.get(user).divide(BigDecimal.valueOf(tarefasConcluidas.get(user)),
                                                                     BigDecimal.ROUND_UP));
        }
        ...
        for (Tarefa no : tarefas) {
            if (no.getEstado() == Tarefa.CONCLUIDO) {
                ...
                suij = (
                               ...
                               (no.getTempoFinal().get(no.getTempoFinal().size() - 1) - no.getTimeCriacao())
                       ) * (100);
    ...
    private void addMetricasGlobais (MetricasGlobais global) {
        ...
        metricasGlobais.setOciosidadeComputacao(
                metricasGlobais.getOciosidadeComputacao() + global.getOciosidadeComputacao());
        metricasGlobais.setOciosidadeComunicacao(
                metricasGlobais.getOciosidadeComunicacao() + global.getOciosidadeComunicacao());
    ...
    public Object[][] makeResourceTable () {
        ...
        for (int i = 0; i < table.size(); i++) {tableArray[i] = table.get(i);}
    ...
    public void calculaMedia () {
        ...
        System.out.println("Usuário \t SatisfaçãoGeralTempoSim" + "\t" + "SatisfaçãoGeralTempoUso" + "\t" +
                           "SatisfaçãoDesempenho" + "\t" + "ConsumoTotal" + "\t" + "LimiteConsumoTempoSimulado" + "\t" +
                           "LimiteConsumoTempoUso" + "\t" + "ConsumoLocal" + "\t" + "ConsumoLocalProprio" + "\t" +
                           "ConsumoLocalEstrangeiro" + "\t" + "ConsumoMaxLocal" + "\t" + "TarefasPreemp" + "\t" +
                           "Alpha" + "\t" + "BetaTempoSim" + "\t" + "BetaTempoUso" + "\t" + "Desperdicio" + "\t" +
                           "ConsumoKJS" + "\t" + "TurnaroundTime" + "\t" + "TempoSIM");
        for (int j = 0; j < usuarios.size(); j++) {
            for (int i = 0; i < numeroDeSimulacoes; i++) {
                System.out.println(
                        ...
                        + String.format("%.2f",
                                        historicoConsumoTotalUsuario.get(i).get(usuarios.get(j)).doubleValue() / 1000000
                        ) + "\t"
                        + String.format("%.2f",
                                        historicoLimitesConsumoTempoSim.get(i).get(usuarios.get(j)).doubleValue() /
                                        1000000
                        ) + "\t"
                        + String.format("%.2f",
                                        historicoLimitesConsumoTempoUso.get(i).get(usuarios.get(j)).doubleValue() /
                                        1000000
                        ) + "\t"
                        + String.format("%.2f", (
                                                        historicoConsumoLocal.get(i).get(usuarios.get(j))
                                                                             .divide(BigDecimal.valueOf(
                                                                                             historicoTempoSim.get(i)),
                                                                                     BigDecimal.ROUND_UP)
                                                ).doubleValue() / 1000) + "\t"
                        + String.format("%.2f", (
                                                        historicoConsumoLocalProprio.get(i).get(usuarios.get(j))
                                                                                    .divide(BigDecimal.valueOf(
                                                                                                    historicoTempoSim.get(i)),
                                                                                            BigDecimal.ROUND_UP)
                                                ).doubleValue() / 1000) + "\t"
                        + String.format("%.2f", (
                                                        historicoConsumoLocalEstrangeiro.get(i).get(usuarios.get(j))
                                                                                        .divide(BigDecimal.valueOf(
                                                                                                        historicoTempoSim.get(
                                                                                                                i)),
                                                                                                BigDecimal.ROUND_UP)
                                                ).doubleValue() / 1000) + "\t"
                        + String.format("%.2f",
                                        historicoConsumoMaxLocal.get(i).get(usuarios.get(j)).doubleValue() / 1000000
                        ) + "\t"
                        ...
                        + String.format("%.2f",
                                        historicoEnergiaDeperdicada.get(i).get(usuarios.get(j)).doubleValue() / 1000000
                        ) + "\t"
                        + String.format("%.2f", (
                                                        historicoConsumoTotalUsuario.get(i).get(usuarios.get(j))
                                                                                    .divide(BigDecimal.valueOf(
                                                                                                    historicoTempoSim.get(i)),
                                                                                            BigDecimal.ROUND_UP)
                                                ).doubleValue() / 1000) + "\t"
    ...
    private void addMetricaSatisfacao (
            Map<String, Double> metricasSatisfacao, Map<String, Integer> tarefasConcluidasUser
    ) {
        if (numeroDeSimulacoes == 0) {
