...
public class Interpretador {
    ...
    public void geraTraceSim (final Collection<? extends Tarefa> tasks) {
        ...
        try (
            ...
            out.write("""
                      ...
                      </system>""".formatted(this.type, Interpretador.makeTasksDescription(tasks)));
        } catch (final IOException ignored) {
        }
    }

    private static String makeTasksDescription (final Collection<? extends Tarefa> tasks) {
        return tasks.stream()
    ...
    private static String makeTaskDescription (final Tarefa task) {
        return """
               ...
               """.formatted(
                task.getIdentificador(), task.getTimeCriacao(), task.getTamProcessamento(),
                task.getArquivoEnvio(), task.getProprietario()
        );
    }
}